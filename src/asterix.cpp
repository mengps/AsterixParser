...
AsterixBlock::AsterixBlock(const uchar* data, const Uap& uap) :
    m_data(data)
{
    if (!uap.m_categories.contains( category() ))
    {
        qDebug() << ("Cat " + QString::number(category()) + " is undefined");
    ...
    m_index(blockIndex)
{
    ...
    {
        if (!uapCat->m_dataItems.contains(frn))
        {
            qDebug() << ("FRN " + QString::number(frn) +  " is undefined");
    ...
    m_uapDataItem(uapDataItem)
{
    if (m_uapDataItem->format() == UapDataItem::COMPOUND)
    {
        ...
        foreach (int frn, frns)
        {
            if (!uapDataItem->m_subFields.contains(frn))
            {
                qDebug() << ("Subfield FRN " + QString::number(frn) + " is undefined");
                m_length = -1;       // mark this record as invalid
                return; // undefined frn
            }
...
QString AsterixDataItem::decodeIcaoStr(const uchar* p)
{
    ...
    {
        auto ch = decodeIcaoSixBitChar(s >> b);
        if (ch != QChar::Null) ret += ch;
    }
