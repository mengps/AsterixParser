    ...
    m_scale(1.0)
{
    ...
    if (!fieldElm.firstChildElement("unit").text().isEmpty())
    {
        m_unit = unitFromStr(fieldElm.firstChildElement("unit").text());
    }

    if (!fieldElm.firstChildElement("scale").text().isEmpty())
    {
        bool error;
        ...
        if (error)
        {
            qDebug() << (ScaleExpressionParser::lastError + " in <scale> expression '" + fieldElm.firstChildElement("scale").text() + "'");
            m_scale = 1.0;
        }
    ...
    if (m_msb < m_lsb)
    {
        qDebug() << ("Invalid msb/lsb on field '" + m_name + "'");
        m_msb = m_lsb;
    }
...
int UapDataItem::calcLength(const uchar *p) const
{
    ...
    case COMPOUND:
        qDebug() << ("Error in specification: Compound length cannot be computed.");
        break;
    }
...
bool UapDataItem::fieldPresent(const uchar *p, const UapField &uapField) const
{
    ...
    case COMPOUND:
        qDebug() << ("Error in specification: Field defintion not allowed below compound.");
        return false;
        break;
    }
...
void Uap::readXml(QFile* input)
{
    ...
    for (int i = 0; i < catElms.count(); i++)
    {
        ...
        qDebug() << ("Read cat " + QString::number(uapCategory->m_category) + " version " + uapCategory->m_version + " from file '" + uapCategory->m_sourceFile + "'");
    }
}


void Uap::readXml(const QString &dirpath)
{
    QDir dir(dirpath);

    qDebug() << ("Reading *.xml files from directory " + dir.absolutePath());

    QFileInfoList fileInfos = dir.entryInfoList(QStringList() << "*.xml", QDir::Files | QDir::Readable);

    if (fileInfos.isEmpty())
    {
        qDebug() << ("No *.xml files found in " + dir.absolutePath());
    }
    ...
    foreach (QFileInfo fileInfo, fileInfos)
    {
        ...
        {
            qDebug() << ("Unable to open file " + fileInfo.filePath());
            continue;
        }
